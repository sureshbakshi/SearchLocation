...
export default function SearchPlaces() {
    ...
    const [searchHistory, setHistory] = React.useState([])
    ...
    React.useEffect(() => {
        ...
        fetch({ input: inputValue }, (results, status) => {
            if (active) {
                ...
                if (value) {
                    newOptions = [value];
                    updateMap(value.place_id, status)
                }
                if (results) {
    ...
    function addToHistory(searchItem) {
        setHistory(history => [...history, searchItem]);
    }
    return (
        <>
            <div className={styles.google_combo_box}>
                <h2>Search Without Redux</h2>
